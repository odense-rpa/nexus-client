...
def test_hent_skemadefinition_på_forløb(
...
def test_opret_skema_på_borger_uden_forløb(
    nexus_manager: NexusClientManager, test_borger: dict
):
...
def test_opret_skema_på_borger_på_forløb(
    nexus_manager: NexusClientManager, test_borger: dict
):
...
def test_opret_skema_med_tag(nexus_manager: NexusClientManager, test_borger: dict):
    ...
    assert "id" in oprettet_skema
    assert oprettet_skema["formDefinition"]["title"] == skema_navn
    assert any(tag["name"] == tag_navn for tag in oprettet_skema.get("tags", [])), (
        f"Skema should have tag '{tag_navn}'"
    )
...
def test_hent_skema_fra_reference(nexus_manager: NexusClientManager, test_borger: dict):
    ...
    skemareference = next(
        (ref for ref in skemareferencer if ref["Skemaid"] == 12345), None
    )
...
def test_rediger_skema(nexus_manager: NexusClientManager, test_borger: dict):
    ...
    skemaer = filter_by_predicate(
        aktiviter, lambda aktivitet: aktivitet.get("patientActivityType") == "formData"
    )
    skema = filter_by_predicate(skemaer, lambda skema: skema.get("id") == 10274229)
...
def test_hent_koordinator_skema_opret_netværksperson(
    nexus_manager: NexusClientManager, test_borger: dict
):
    ...
    navn_felt = next((item for item in skema["items"] if item["label"] == "Navn"), None)
    organisationstilknytning_felt = next(
        (
            item
            for item in skema["items"]
            if item["label"] == "Organisationstilknytning"
        ),
        None,
    )
    stillingsbetegnelse_felt = next(
        (item for item in skema["items"] if item["label"] == "Stillingsbetegnelse"),
        None,
    )

    if (
        not navn_felt
        or not organisationstilknytning_felt
        or not stillingsbetegnelse_felt
    ):
    ...
    assert response_json is not None, (
        "Response from opret_netværksperson should not be None"
    )
