...
class TestFindNodes:
    ...
    def test_find_nodes_finds_all_matching(self, sample_tree):
        ...
        # Find all leaf nodes
        leaf_nodes = find_nodes(sample_tree, lambda node: node.get("type") == "leaf")
    ...
    def test_find_nodes_finds_first_only(self, sample_tree):
        ...
        leaf_nodes = find_nodes(
            sample_tree, lambda node: node.get("type") == "leaf", find_all=False
        )
    ...
    def test_find_nodes_with_list_input(self, sample_tree):
        ...
        leaf_nodes = find_nodes(children, lambda node: node.get("type") == "leaf")
    ...
    def test_find_first_node(self, sample_tree):
        ...
        first_leaf = find_first_node(
            sample_tree, lambda node: node.get("type") == "leaf"
        )
    ...
    def test_find_first_node_no_match(self, sample_tree):
        ...
        result = find_first_node(
            sample_tree, lambda node: node.get("type") == "nonexistent"
        )
...
class TestFilterByPath:
    ...
    def test_filter_by_path_nexus_data(
        self, test_borger: dict, nexus_manager: NexusClientManager
    ):
        ...
        grundforløb = filter_by_path(
            referencer, "/ÆHF - Forløbsindpla%", active_pathways_only=True
        )
        assert len(grundforløb) == 1
    ...
    def test_filter_by_path_basic(self, pathway_tree):
        ...
        results = filter_by_path([pathway_tree], "/Sundhedsfagligt grundforløb/FSIII")
    ...
    def test_filter_by_path_wildcard(self, pathway_tree):
        ...
        results = filter_by_path(
            [pathway_tree], "/Sundhedsfagligt grundforløb/*/Medicin%"
        )
    ...
    def test_filter_by_path_active_pathways_only(self, pathway_tree):
        ...
        results = filter_by_path(
            [pathway_tree], "/Sundhedsfagligt grundforløb", active_pathways_only=True
        )
...
class TestFilterByPredicate:
    ...
    def test_filter_by_predicate_complex(self, sample_tree):
        ...
        high_value_nodes = filter_by_predicate(
            [sample_tree], lambda node: node.get("value", 0) > 15
        )
    ...
    def test_filter_by_predicate_custom_children_key(self):
        ...
        grants = filter_by_predicate(
            [tree], lambda node: node.get("type") == "grant", children_key="subcatalogs"
        )
...
class TestMapTree:
    ...
    def test_map_tree_preserves_structure(self, sample_tree):
        ...
        assert len(transformed["children"][0]["children"]) == len(
            sample_tree["children"][0]["children"]
        )
