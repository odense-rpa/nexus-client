...
def test_hent_kalender(kalender_client: KalenderClient, test_citizen: dict):
    borger = test_citizen
    kalender = kalender_client.hent_kalender(borger)
    assert kalender is not None

def test_get_citizen_calendar_backward_compatibility(kalender_client: KalenderClient, test_citizen: dict):
...
def test_hent_begivenheder(kalender_client: KalenderClient, test_citizen: dict):
...
def test_events_backward_compatibility(kalender_client: KalenderClient, test_citizen: dict):
    """Test backward compatibility for old method name."""
    citizen = test_citizen
    citizen_calendar = kalender_client.get_citizen_calendar(citizen)
