...
def get_test_element(grant_prototype_data, element_type, as_list=True):
    ...
    element = next(
        (
            e
            for e in grant_prototype_data["currentElements"]
...
def test_text_fields_success_and_exception(nexus_manager, grant_prototype_data):
    ...
    with pytest.raises(
        ValueError, match="Field 'personReference' expects a string value"
    ):
        client._fill_grant_elements(elements, fields)
...
def test_value_with_possible_values_success_and_exception(
    ...
    with pytest.raises(
        ValueError,
        match="Value 'INVALID_PAYOR' not in possible values for field 'payor'",
    ):
        client._fill_grant_elements(elements, fields)
...
def test_boolean_value_fields_success_and_exception(
    nexus_manager, grant_prototype_data
):
    ...
    with pytest.raises(
        ValueError,
        match="Field 'denmarkStatisticRareDisability' expects a boolean value",
    ):
        client._fill_grant_elements(elements, fields)
...
def test_selected_values_success_and_exception(nexus_manager, grant_prototype_data):
    ...
    # Get test element
    elements = get_test_element(
        grant_prototype_data, "denmarkStatisticMappingTargetGroup"
    )
    ...
    with pytest.raises(
        ValueError,
        match="Field 'denmarkStatisticMappingTargetGroup' expects a list of selected values",
    ):
        client._fill_grant_elements(elements, fields)
    ...
    with pytest.raises(
        ValueError, match="Selected value 'INVALID_VALUE' not in possible values"
    ):
        client._fill_grant_elements(elements, fields)
    ...
    with pytest.raises(
        ValueError,
        match="Selected value '123' in field 'denmarkStatisticMappingTargetGroup' must be a string",
    ):
        client._fill_grant_elements(elements, fields)
...
def test_supplier_fields_success_and_exception(nexus_manager, grant_prototype_data):
    ...
    with patch.object(client.client, "get") as mock_get:
        ...
        with pytest.raises(
            ValueError, match="Supplier 'Nonexistent Supplier' not found"
        ):
            client._fill_grant_elements(elements, fields)
        ...
        with pytest.raises(
            ValueError, match="Field 'supplier' expects a supplier name"
        ):
            client._fill_grant_elements(elements, fields)


def test_fill_grant_elements_modifies_original_data(
    nexus_manager, grant_prototype_data
):
    ...
    fields = {"personReference": "test-modified-person-123", "plannedDate": test_date}
