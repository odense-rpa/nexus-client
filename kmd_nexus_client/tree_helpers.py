...
def find_node_by_id(
    ...
    return find_first_node(
        roots, 
        lambda node: node.get(id_key) == node_id,
        children_key
    )
...
def filter_by_path(
    ...
    for root in roots:
        results.extend(_filter_tree_by_path(
...
def _filter_tree_by_path(
    ...
    if (active_pathways_only and 
        node.get("type") == "patientPathwayReference" and 
        node.get("pathwayStatus") != "ACTIVE"):
        return result
    ...
    if _path_matches(current_path, target_path):
        ...
        if children and len(target_path) > len(current_path):
            for child in children:
                result.extend(_filter_tree_by_path(
                    child, current_path, target_path, active_pathways_only, children_key
                ))
...
def map_tree(
    ...
    if children:
        transformed[children_key] = [
            map_tree(child, transform_fn, children_key) 
            for child in children
        ]
