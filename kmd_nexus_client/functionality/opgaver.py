...
class OpgaverClient:
    ...
    def hent_opgave_for_borger(self, borger: dict, opgave_id: int) -> Optional[dict]:
        ...
        response = self.nexus_client.get(f"assignments/patient/{borger["id"]}/assignments?ids={opgave_id}")
    ...
    def hent_opgavetyper(self, objekt: dict) -> List[dict]:
        ...
        response = self.nexus_client.get(objekt["_links"]["availableAssignmentTypes"]["href"])
        return response.json()
    
    def opret_opgave(self, objekt: dict, opgave_type: str, titel: str, ansvarlig_organisation: str, start_dato: date, forfald_dato: Optional[date] = None, beskrivelse: Optional[str] = None, ansvarlig_medarbejder: Optional[dict] = None) -> dict:
        """
        Opret en ny opgave.
        ...
        found_assignment_type = next(
            ...
            if item["name"] == opgave_type),None
        )

        if not found_assignment_type:
            raise ValueError(f"Opgave type {opgave_type} ikke fundet i tilgængelige opgave typer.")

        # Get prototype
        prototype = self.nexus_client.get(found_assignment_type["_links"]["assignmentPrototype"]["href"]).json()
        ...
        if not probable_organization:
            raise ValueError(f"Organisation {ansvarlig_organisation} ikke fundet i tilgængelige organisationer.")
    ...
    def rediger_opgave(self, opdateret_opgave: dict) -> dict:
        ...
        """
        try:
            handling = next((a for a in opdateret_opgave["actions"] if a.get("name") == "Gem"), None)
    ...
    def luk_opgave(self, opgave: dict) -> bool:
        ...
        try:
            ...
            response = self.nexus_client.put(
                afslut_action["_links"]["updateAssignment"]["href"],
                json=opgave
            )
