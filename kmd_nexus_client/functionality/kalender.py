...
class KalenderClient:
    ...
    def hent_kalender(
        self, borger: dict, kalender_navn: str = "Borgerkalender"
    ) -> dict:
        """
        ...
        calendar = next(
            (
                ...
                if item["name"] == kalender_navn
            ),
            None,
        )
        if not calendar:
            raise ValueError(
                f"Kalender {kalender_navn} ikke fundet i borgerens præferencer."
            )
    ...
    def hent_begivenheder(
        self, kalender: dict, start_dato: date, slut_dato: date
    ) -> dict:
        """
