...
class AktivitetslisteClient:
    ...
    def hent_aktivitetsliste(
        ...
        aktivitetsliste = next(
            (
                item
                for item in præferencer.get("ACTIVITY_LIST", [])
                if item.get("name") == navn
            ),
            None,
        )
        ...
        aktivitetsliste = self.client.get(
            aktivitetsliste["_links"]["self"]["href"]
        ).json()
        ...
        if organisation and medarbejder:
            content_url = (
                base_content_url + f"&pageSize={antal_sider}"
